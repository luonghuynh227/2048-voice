	...
	slide: function (direction) {
				...
				if (cell.number !== null) {
					...
					for (let k = j - 1; k >= 0; k--) {
						...
						if (foreCell.number === null) {
							...
							moveToCell = foreCell;
						} else if (cell.number.dataset.value === foreCell.number.dataset.value && cell.number.dataset.value < variable.value_stop) {
							// the cell has same number, move, merge and stop
							moveToCell = foreCell;
							
							// display score
							let combinedTotal = parseInt(cell.number.dataset.value);
							variable.score+=combinedTotal;
							variable.scoreDisplay.innerHTML = variable.score;
		...
		setTimeout(function () {
			if (number.spawn()) {
				...
				if (variable.score > localStorage.getItem('gameHighScore')) {
					localStorage.setItem('gameHighScore', variable.score)
					variable.hightScoreDisplay.innerHTML = variable.score
				}
				// const numberElements = document.getElementsByClassName("number");
				// grid.gridElement.removeChild(numberElements);
				// number.numberDiv.parentNode.removeChild(element);
				// grid.init();
			}
		}, 50)
	}
